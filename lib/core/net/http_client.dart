  ...
  Future<Either<AppErrors, T>> sendRequest<T extends BaseModel> ({
    ...
    try {
      switch (method) {
        case HttpMethod.GET:
          response = await _client.get(
            ...
            options:url == APIUrls.API_CHECK_COUPON_VALIDITY ? Options(headers: headers) : CacheOptions(store: MemCacheStore(),priority:  CachePriority.high,maxStale: const Duration(days: 7),hitCacheOnErrorExcept: [401, 403], policy: CachePolicy.forceCache,).toOptions(),
            cancelToken: cancelToken,
          );
          break;
      ...
      if (responseValidator.isValid) {
        ...
        ///
        try {
          if(T is WeatherModel)
              response.data.toString().logD;
